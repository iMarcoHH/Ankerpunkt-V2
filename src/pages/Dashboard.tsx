...
import { TrendingUp, TrendingDown, Wallet, Target, ArrowRightLeft, ShieldCheck } from 'lucide-react'
...
export function DashboardPage() {
  ...
  const monthlyInsurance = useMemo(() => insurances.reduce((s, i) => s + (i.period === 'monthly' ? i.amount : i.amount / 12), 0), [insurances])
  ...
  return (
    <div className="p-5 space-y-5 pb-8">
      ...
      <div className="flex items-center justify-between pt-14">
        <button onClick={goToPrevMonth} className="w-9 h-9 rounded-full flex items-center justify-center transition-colors"
        ...
        <button onClick={goToNextMonth}
          ...
          style={{ background: isCurrentMonth ? 'rgba(255,255,255,0.02)' : 'rgba(255,255,255,0.06)',
                   color: isCurrentMonth ? 'rgba(154,160,166,0.3)' : '#9AA0A6' }}
      ...
      <motion.div
        ...
        <p className="text-4xl font-display tracking-wide text-white mt-2"
           style={{ color: balance >= 0 ? 'white' : '#C8392B' }}>
          {fmt(balance)}
        </p>
        <div className="mt-4 flex items-center gap-4">
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-signal"/>
            <span className="text-xs text-cement">Einnahmen {fmt(totalIncome)}</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-red"/>
            <span className="text-xs text-cement">Ausgaben {fmt(totalExpense)}</span>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          { label: 'Versicherung', value: fmt(monthlyInsurance)+'/mo',Icon: ShieldCheck,  accent: '#3D5166' },
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <motion.div className="ak-card p-5" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          ...
          <div style={{ height: 200 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={trend} margin={{ top: 4, right: 0, left: -20, bottom: 0 }}>
                ...
                <Tooltip
                  contentStyle={{ backgroundColor: '#162030', borderColor: '#3D5166', borderRadius: 12, color: '#E8DFD0', fontSize: 12 }}
          ...
          <div className="flex gap-4 mt-3">
            <div className="flex items-center gap-1.5 text-xs text-cement">
              <span className="w-3 h-3 rounded-sm" style={{ background: '#E8A832' }}/> Einnahmen
            </div>
            <div className="flex items-center gap-1.5 text-xs text-cement">
              <span className="w-3 h-3 rounded-sm" style={{ background: '#C8392B' }}/> Ausgaben
            </div>
          </div>
        </motion.div>
        ...
        <motion.div className="ak-card p-5" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setActiveTab('buchungen')}
              className="text-xs text-cement hover:text-white transition-colors">Alle →</button>
          </div>
          {recentTx.length === 0 ? (
      ...
      <motion.div className="grid grid-cols-3 gap-3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
        {[
          ...
          { label: `${achievements.length} Erfolge`, tab: 'gamification', Icon: Trophy as any, color: '#C8392B' },
        ].map(({ label, tab, Icon, color }) => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className="ak-card p-4 flex flex-col items-center gap-2 transition-colors hover:bg-card2">
