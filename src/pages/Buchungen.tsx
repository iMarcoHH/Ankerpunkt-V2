...
export function BuchungenPage() {
  ...
  // Gefilterte Transaktionen für den ausgewählten Monat
    ...
    <div className="pb-24 min-h-screen" style={{ background: '#F4F2EE' }}>
      ...
      <div className="bg-navy px-5 pt-14 pb-5" style={{ borderBottom: '3px solid #C8392B' }}>
        ...
        <div className="flex items-center justify-between">
          <div className="font-display text-white text-4xl tracking-wide">BUCHUNGEN</div>
          <button
            onClick={() => tab === 'wiederkehrend' ? setShowAddRecurring(true) : setShowAdd(true)}
            className="w-9 h-9 rounded-full flex items-center justify-center"
            style={{ background: '#C8392B', boxShadow: '0 4px 12px rgba(200,57,43,0.4)' }}>
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <div className="text-center">
            <div className="font-display text-white text-xl tracking-wide">{MONTH_NAMES[viewMonth]}</div>
      ...
      <div className="px-4 py-4 space-y-3">
        ...
        <div className="flex gap-1">
          {([['ausgaben','Ausgaben'],['einnahmen','Einnahmen'],['wiederkehrend','Wiederkehrend']] as const).map(([val, label]) => (
            <button key={val} onClick={() => setTab(val)}
              className={`ak-tab flex-1 ${tab === val ? 'active' : ''}`} style={{ fontSize: 9 }}>{label}</button>
          ))}
        </div>
          ...
          <>
            ...
            <div className="ak-card p-4 flex items-center justify-between"
              ...
              <div className="font-mono text-[10px] text-cement tracking-widest uppercase">
                {tab === 'ausgaben' ? 'Gesamtausgaben' : 'Gesamteinnahmen'} · {MONTH_NAMES[viewMonth]}
              </div>
            ...
            {monthTx.length === 0 ? (
              <div className="ak-card p-8 text-center">
                ...
                <p className="font-sans text-sm text-cement">Keine {tab === 'ausgaben' ? 'Ausgaben' : 'Einnahmen'} in diesem Monat.</p>
              </div>
            ) : (
              <div className="space-y-2">
                {monthTx.map(tx => (
                  <div key={tx.id} className="ak-card p-4"
        ...
        {tab === 'wiederkehrend' && (
          <>
            <div className="ak-card bg-navy/5 p-3" style={{ borderLeft: '3px solid #E8A832' }}>
            ...
            {recurring.length === 0 ? (
              <div className="ak-card p-8 text-center">
                ...
                <p className="font-sans text-xs text-cement mt-1">z.B. Miete, Gehalt, Abos</p>
              </div>
            ) : (
              <div className="space-y-2">
                {recurring.map(r => (
                  <div key={r.id} className="ak-card p-4"
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full flex items-center justify-center font-semibold text-sm"
                             style={{ background: r.type === 'income' ? 'rgba(232,168,50,0.12)' : 'rgba(200,57,43,0.1)',
                                      color: r.type === 'income' ? '#E8A832' : '#C8392B' }}>
                          🔁
                        </div>
                        <div>
                          <div className="font-sans text-sm font-semibold text-navy">{r.description}</div>
                          <div className="font-mono text-[9px] text-cement uppercase tracking-wider mt-0.5">
                            {r.category} · jeden {r.day_of_month}. des Monats
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => handleToggleRecurring(r.id)}
                          ...
                          style={{ background: r.active ? '#0D1B2A' : 'rgba(0,0,0,0.05)',
                                   color: r.active ? 'white' : '#9AA0A6',
                                   WebkitTapHighlightColor: 'transparent' }}>
                          {r.active ? '✓' : '○'}
                        </button>
      ...
      {showAdd && <AddSheet onClose={() => setShowAdd(false)} type={tab === 'ausgaben' ? 'expense' : 'income'}/>}

      {/* Add Recurring Sheet */}
      {showAddRecurring && <AddRecurringSheet onClose={() => setShowAddRecurring(false)}/>}
    </div>
  )
}

// ── Add Transaction Sheet ────────────────────────────────────────────────────
function AddSheet({ onClose, type: initialType }: { onClose: () => void; type: 'income' | 'expense' }) {
  const { transactions, setTransactions, userId, viewMonth, viewYear } = useStore()
  const [type, setType] = useState(initialType)
  const [amount, setAmount]     = useState('')
  const [desc, setDesc]         = useState('')
  const [category, setCategory] = useState('')
  const [date, setDate]         = useState(
    new Date(viewYear, viewMonth, new Date().getDate()).toISOString().split('T')[0]
  )
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-sheet">
        <div className="flex justify-center mb-4"><div className="w-10 h-1 rounded-full bg-navy/15"/></div>
        <div className="flex justify-between items-center mb-5">
          <div className="font-display text-navy text-2xl">NEUER EINTRAG</div>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-navy/6 flex items-center justify-center text-cement text-xl">×</button>
        </div>
        ...
        <div className="flex gap-2 mb-4">
          {(['expense','income'] as const).map(t => (
            <button key={t} onClick={() => { setType(t); setCategory('') }}
              ...
              style={{ background: type === t ? (t === 'expense' ? '#C8392B' : '#E8A832') : '#F4F2EE',
                       color: type === t ? 'white' : '#9AA0A6' }}>
...
function AddRecurringSheet({ onClose }: { onClose: () => void }) {
  ...
  async function handleSave() {
    ...
    const entry: Omit<RecurringEntry, 'id' | 'created_at'> = {
      ...
      description: desc, category, day_of_month: parseInt(day), active: true
    }
    if (userId) {
      const { data: row } = await supabase.from('recurring_entries').insert(entry).select().single()
      if (row) setRecurring([...recurring, row])
    } else {
      setRecurring([...recurring, { ...entry, id: Date.now().toString(), created_at: new Date().toISOString() }])
    }
    setSaving(false)
    onClose()
  }
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-sheet">
        <div className="flex justify-center mb-4"><div className="w-10 h-1 rounded-full bg-navy/15"/></div>
        <div className="flex justify-between items-center mb-5">
          <div className="font-display text-navy text-2xl">WIEDERKEHREND</div>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-navy/6 flex items-center justify-center text-cement text-xl">×</button>
        </div>
        ...
        <div className="space-y-3">
          ...
          <input className="ak-input" placeholder="Beschreibung (z.B. Miete)" value={desc} onChange={e => setDesc(e.target.value)}/>
